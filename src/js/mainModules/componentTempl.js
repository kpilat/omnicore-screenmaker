    ...
    ${component.id}.onclick = async function () {
        ${
            component.alertType
                ...
                : (component.action === 'set-value' ? Config.setValue(component) : '',
                  component.action === 'toggle-signal' ? Config.toggleSignal(component) : '')
        }
    };
    ${component.targetType === 'signal' ? Config.subscribeSignal(component) : ''}
    ${component.targetType === 'rapid' ? Config.subscribeVariable(component) : ''}
    try {
    ...
    const newComponent = /*javascript*/`
        ...
        ${component.id}.onclick = async function () {
            ${
                component.alertType
                    ? Config.alertPopup(component)
                    : (component.action === 'increase-value' ? Config.increaseValue(component) : '',
                      component.action === 'decrease-value' ? Config.decreaseValue(component) : '',
                      component.action === 'set-value' ? Config.setValue(component) : '',
                      component.action === 'toggle-signal' ? Config.toggleSignal(component) : '')
            }
        };
...
const inputInit = (component) => {
    ...
    ${component.id}.onchange = async function (newValue) {
        ${component.alertType ? Config.alertPopup(component) : component.action === 'set-string' ? Config.setString(component) : ''}
    }
...
const radioInit = (component) => {
    ...
    ${component.radioGroup}.${component.id}.onclick = async function () {
        ${component.alertType ? Config.alertPopup(component) : component.action === 'set-value' ? Config.setRadioButton(component) : ''}
    }

    ${Config.subscribeVariableRadio(component)}

	try {
		const initValue = await ${component.radioGroup}.${component.id + component.id}.getValue();
...
const labelInit = (component) => {
    if (!component.callback && !component.target) {
        return '';
    }
    console.log(component);

    const newComponent = /*javascript*/`
        ${component.id} = document.querySelector('#${component.id} .text');
        ${component.id}['originalText'] = '${component.text}';

        ${Config.subscribeVariable(component)}

        ${component.id + component.id}.addCallbackOnChanged( (newValue) => {
            ${component.id}.innerHTML = ${component.id}.originalText.replace('$', newValue.slice(1, -1));
        });

        try {
            let newValue = await ${component.id + component.id}.getValue();
            ${component.id}.innerHTML = ${component.id}.originalText.replace('$', newValue);
        } catch (e) {
            FPComponents.Popup_A.message(e.message, [e.httpStatus.code, e.controllerStatus.name, e.controllerStatus.description]);
        }
  `;
